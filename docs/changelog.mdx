...
- **Chat no longer reports "网络错误" when you run out of monthly tokens mid-answer.** The streaming endpoints used to drop the connection when the per-user token quota was exhausted partway through a response, and the frontend interpreted the abrupt cutoff as a network failure — leaving you with no way to know upgrading was the actual fix. The stream now emits a structured `QUOTA_EXCEEDED` terminator with current usage, monthly cap, next reset date, and plan, and the playground surfaces a dialog with an "Upgrade now" deep-link to your billing settings instead of a generic toast.
