...
- **Celebration dialog when a Stripe checkout succeeds.** Returning from Stripe now opens a centered "Welcome aboard!" modal that names the purchased plan and fires confetti, instead of a stray toast that was easy to miss. The dialog dismisses on Got it / Esc / backdrop, and only then are the redirect params (`status`, `plan`, `session_id`) stripped from the URL — so a refresh during the moment doesn't re-trigger it.
- **Admin-controlled billing feature flag.** New `Enable Stripe Billing` toggle in Admin -> System Settings gates the entire Stripe pipeline — user-facing `/api/billing/*` endpoints, the Stripe webhook, the admin plan/subscription CRUD, and the frontend `Plan & Billing` tab and admin nav group all return 503 (or hide entirely) when billing is off. Activation is a one-time idempotent procedure that seeds the Free + Pro catalogue, sets the `default_plan_id` pointer, and backfills existing users; toggling off/on afterwards is a pure flag flip with no data side-effects, so private deployments without Stripe credentials never surface a non-functional payment UX. The quota chain now skips the per-user plan tier when billing is off, so legacy installs continue to honour the admin-set `Default Monthly Token Quota`. `STRIPE_BILLING_RETURN_URL` default updated to `/settings?tab=billing` and the Checkout redirect now correctly preserves any existing query string in that URL.
- **Stripe billing data model + SDK groundwork.** Introduces the `billing_plans` / `subscriptions` / `stripe_webhook_events` tables and seeds Free + Pro plans (Pro is currently a Stripe **test mode** price). No payment UI yet — checkout, Customer Portal, webhook handling, and the `/settings/billing` page land in follow-ups.
- **Subscribe to Pro plan via Stripe Checkout.** New billing API surfaces plan listings, Checkout / Customer Portal redirects, and a Stripe webhook so subscriptions provision and renew automatically. Quota enforcement now respects each user's billing plan, and canceled subscriptions stay on Pro until the paid period ends.
- **Settings -> Billing page.** Shows the current plan, monthly token usage with a progress bar, period reset date, and subscription status. The `Upgrade to Pro` button redirects to Stripe Checkout; existing paid users get a `Manage subscription` button that opens the Stripe Customer Portal. The mid-stream quota dialog now lands here instead of 404'ing.
- **Admin billing management.** Admins can now create, edit, soft-delete, and inspect billing plans, plus browse all Stripe subscriptions with status / plan / email filters. New entries live under Admin Panel -> Billing.
...
- **Browser tab title now updates when switching tabs in `/settings` and `/admin`.** Previously the `<title>` stayed frozen at the initial page name regardless of which sub-section (Account, Billing, Users, Plans, etc.) the user navigated into, making it impossible to distinguish multiple pinned tabs of the portal. The active sub-tab is now reflected as `Settings · Billing — FIM One` in the tab title, matching the format already used by the rest of the app.
- **Usage page no longer reports "Unlimited" when an admin has set a default token quota.** The user Usage card was reading `users.token_quota` directly, ignoring both the user's billing plan and the system-wide `default_token_quota`. It now resolves through the unified quota chain (per-user override > plan tier > system default), and admins who set `Default Monthly Token Quota` see that limit propagate to the Free plan automatically.
...
- **Plan & Billing is now a Settings tab, not a standalone page.** Folded `/settings/billing` back into `/settings?tab=billing` so the left nav stays visible while users review usage and switch plans — matching how GitHub, Linear, and Notion ship their billing UX. The standalone route now redirects, so old bookmarks and Stripe Checkout return URLs from earlier deploys keep working. The `Subscriptions` tab is renamed to `Marketplace` to disambiguate it from the new plan subscription.
- **Token quotas now render as `5M` / `100K` instead of `5,000,000`.** A new `formatTokens` helper produces compact labels across the billing page, plan cards, and the admin Plans table — admin INPUT fields still accept raw numbers.
- **Plan & Billing usage card no longer duplicates the Usage tab.** The card stays a thin progress + reset summary, with a `View detailed usage history →` link that deep-links to the dedicated Usage tab for per-agent and daily breakdowns.
- **Free plan quota is now read-only on the admin Plans page.** It is sourced from System Settings -> Default Monthly Token Quota; editing the quota field there propagates to the Free plan automatically. The admin Plans form locks the input (with a deep link back to System Settings) and the backend silently ignores `monthly_token_quota` on PATCH `/admin/billing/plans/{free.id}` so curl users get a forgiving no-op rather than an error.
