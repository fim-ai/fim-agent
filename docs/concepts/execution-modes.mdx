...
FIM One deliberately does **not** build a drag-and-drop workflow editor. This is a strategic choice:
...
4. **Connector = API call.** Complex workflow operations (transfer, reject, escalate) are the target system's responsibility. From the connector's perspective, each operation is just an HTTP request with parameters. FIM One calls the API; the target system manages the state machine.
