---
title: "扩展指南"
description: "如何向 FIM One 接入自定义 LLM 提供商、工具、记忆后端、嵌入模型、重排序器、网络后端以及 RAG 组件。"
---

## 概述

FIM One 围绕一组轻量级抽象基类构建——每个可替换组件对应一个基类。每个组件职责单一、接口最小。你只需实现抽象方法，将实例注入对应的注册器或构造函数，系统其余部分就会自动使用你的实现。
...
将文件放入 `src/fim_one/core/tool/builtin/`，`discover_builtin_tools()` 扫描器会自动找到所有具体的（非抽象的）`BaseTool` 子类——无需手动注册。
