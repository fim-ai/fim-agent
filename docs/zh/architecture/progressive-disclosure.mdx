...
LLMs 在两种货币上支付上下文成本：令牌和注意力。系统提示中注入的每个工具定义都会消耗两者。单个 MCP 服务器可以暴露 90+ 个工具。五个 API 连接器，每个有 20 个操作，会产生 100 个工具定义。三个数据库连接器，每个有 30 个表，会生成另外 90 个模式描述。在用户甚至输入一个词之前，系统提示就可以消耗 50--100KB 的上下文 -- 128K 模型预算的一半。

成本不仅仅是令牌。研究和实践一致表明，**随着无关上下文的增长，LLM 准确性会下降。** 系统提示中有 80 个工具定义的智能体在工具选择上的表现明显不如只有 6 个工具的智能体。模型将注意力花在它永远不会使用的工具模式上，削弱了它对重要工具和指令的关注。

天真的解决方案 -- 注入所有内容，让模型自己排序 -- 无法扩展。FIM One 采取相反的方法：**向 LLM 展示做出决策所需的最少信息，并让它在需要时请求更多。**

## 模式

渐进式信息披露在所有资源类型中遵循两层架构：
...
关键洞察：**完整工具模式是对话范围内的，而不是提示范围内的。** 它们作为工具结果消息出现，上下文管理系统可以在后续轮次中对其进行总结或截断。相比之下，系统提示内容在整个对话中以完整大小持续存在。
...
|---|---|---|---|---|---|
| 技能 | `read_skill` | 名称 + 描述（120 字符） | 完整 SOP 内容 + 嵌入脚本 | `SKILL_TOOL_MODE` | `progressive` |
| API 连接器 | `connector` | 连接器名称 + 操作列表 | 完整操作模式及参数 | `CONNECTOR_TOOL_MODE` | `progressive` |
| 数据库连接器 | `database` | 数据库名称 + 表名 + 计数 | 列模式、SQL 查询执行 | `DATABASE_TOOL_MODE` | `progressive` |
...
**旧版模式：** `SKILL_TOOL_MODE=inline` 将完整的技能内容直接嵌入系统提示中。适用于技能数量少、规模小的情况 -- 但扩展性差。

**上下文节省：** 一个包含 10 个技能、平均每个 2,000 个令牌的部署在渐进模式下消耗约 300 个令牌（仅存根），而在内联模式下消耗约 20,000 个令牌。这是持久上下文成本的 98% 减少。
...
每个连接器存根列出操作名称，但不包含参数架构。LLM 知道*存在哪些*操作，但不知道*如何*调用它们 -- 这正是决定是否使用连接器的适当细节级别。

**按需发生的情况：** `connector("discover", "salesforce")` 返回完整的操作架构，包括 HTTP 方法、URL 路径、参数 JSON 架构和请求体模板。`connector("execute", "salesforce", "get_contacts", {"limit": 10})` 通过 `ConnectorToolAdapter` 代理执行，具有完整的身份验证注入和审计日志记录。

**遗留模式：** `CONNECTOR_TOOL_MODE=legacy` 将每个操作注册为单独的工具（`salesforce__get_contacts`、`salesforce__create_lead` 等）。具有 20 个操作的连接器在系统提示中变成 20 个工具定义。

**上下文节省：** 具有 15 个操作的连接器生成约 50 个令牌的存根，而完整架构约 3,000 个令牌。五个连接器：约 250 个令牌渐进式 vs. 约 15,000 个令牌遗留模式。
...
**旧版模式：** `DATABASE_TOOL_MODE=legacy` 为每个数据库注册三个工具（`{db}__list_tables`、`{db}__describe_table`、`{db}__query`）。使用 5 个数据库连接器，这是 15 个工具定义而不是 1 个。

**上下文节省：** 具有 30 个表和 200 列的数据库生成约 80 个令牌的存根，而完整架构则生成约 5,000 个令牌。节省会随着多个数据库而增加。
...
**按需发生的情况：** `mcp("discover", "github")` 返回完整的工具目录和参数架构。`mcp("call", "github", "create_issue", {"title": "Bug report", "body": "..."})` 委托给存储的 `MCPToolAdapter`，它与 MCP 服务器进程通信。

**传统模式：** `MCP_TOOL_MODE=legacy` 将每个 MCP 工具注册为单独的工具（`github__create_issue`、`github__list_repos` 等）。这很容易超过工具选择阈值并触发不必要的选择阶段。

**上下文节省：** 这里的节省是极端的。GitHub MCP 服务器的 35 个工具可能消耗 10,000+ 个 token 的架构。在渐进式模式中，存根成本约为 100 个 token。如果用户在该对话中从不需要 GitHub，那么这 10,000 个 token 永远不会被消耗。
...
**无环境变量：** 当工具选择过滤集合时，此机制自动激活。没有 `REQUEST_TOOLS_MODE` 环境变量。如果要完全禁用工具选择，请将 `REACT_TOOL_SELECTION_THRESHOLD` 设置为一个非常高的数字。

**上下文节省：** 节省取决于有多少工具可用以及选择选择了多少工具。拥有 30 个工具但仅看到 6 个活跃模式 + `request_tools` 目录的智能体节省大约 60--70% 的工具模式上下文。

## 它如何融入工具组装管道

[系统概述](/architecture/system-overview)描述了一个每个请求8步的工具组装管道。渐进式披露在多个点起作用：

| 管道步骤 | 渐进式披露角色 |
|---|---|
| **1. 基础发现** | 无影响 -- 内置工具正常加载 |
| **2. 智能体类别过滤** | 无影响 -- 类别过滤无论模式如何都适用 |
| **3. KB 注入** | 无影响 -- KB 工具本质上是轻量级的（1-2 个工具） |
| **4. 连接器加载** | `ConnectorMetaTool` 整合所有 API 连接器；`DatabaseMetaTool` 整合所有数据库连接器 |
| **5. MCP 加载** | `MCPServerMetaTool` 将所有 MCP 服务器整合为一个工具 |
| **6. 技能注入** | `ReadSkillTool` 在系统提示中用紧凑存根替换完整内容 |
| **7. CallAgent 注册** | 无影响 -- `call_agent` 已经是一个带有目录的单一工具 |
...
这意味着你可以全局运行 `progressive`（默认值）并为特定智能体有选择地覆盖。具有单个小技能的智能体可能使用 `inline` 模式。需要 LLM 预先查看所有连接器操作的智能体（例如，不能可靠地调用元工具的微调模型）可能使用 `legacy` 模式。
...
一个替代设计是让框架基于启发式方法自动扩展工具模式——例如，检测用户查询涉及的连接器，并在LLM看到提示之前注入其模式。FIM One 有意选择了LLM驱动的方法，原因有三：

1. **LLM在意图检测方面优于启发式方法。** 像"检查客户是否有未解决的工单并更新其档案"这样的查询涉及两个连接器。基于关键字的启发式匹配很脆弱；LLM能自然地识别两者。

2. **透明性。** 当LLM调用 `connector("discover", "jira")` 时，该操作会出现在工具追踪中。用户（以及进行调试的开发人员）可以准确看到加载了哪些模式以及何时加载。隐式扩展是不可见的。

3. **上下文效率。** 框架无法知道LLM需要连接器中的哪些操作。扩展连接器的所有操作会浪费令牌在无关的操作上。LLM首先看到操作名称（通过存根），然后仅请求特定操作的模式——这是两层披露的最纯粹形式。
...
- **枚举约束。** 每个元工具在模式中将其有效名称（连接器名称、数据库名称、服务器名称）列为枚举值。LLM 无法虚构连接器名称。
- **类别语义。** `connector` 工具具有类别 `connector`，`database` 具有类别 `database`，`mcp` 具有类别 `mcp`。这会进入智能体类别过滤——配置为仅 `connector` 类别的智能体将看不到 `database` 或 `mcp` 元工具。

### 为什么同时支持渐进式和传统模式？

并非所有 LLM 都能同样好地处理元工具。较小的或经过微调的模型可能在两步发现-然后-执行的模式中遇到困难。传统模式提供了一个直接的备选方案，其中每个操作都是一个独立的工具，具有完整的架构可见 -- 无需元工具间接调用。

双模式设计也支持迁移。现有部署可以逐步切换到渐进式模式，通过更改单个环境变量一次测试一个资源类型。
