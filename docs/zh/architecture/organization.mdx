...
FIM One 中的每个资源最初都是**个人的**（仅对其创建者可见）。当你将资源发布到组织时，它会变成**可发现的**，组织内的其他成员可以通过市场的组织范围找到它。成员浏览组织的共享资源，并订阅他们需要的资源。
...
组织和全球市场共享相同的基于订阅的访问模型。关键区别在于信任：组织代表一个团队或公司，其中成员彼此认识和信任，因此审查是可选的，凭证共享是直接的。

## 创建和管理组织

每个用户可以创建**无限个**组织并加入任意数量的组织。一个组织有三个角色：
...
当您将资源发布到您的组织时，它**不会**自动出现在每个成员的工作区中。相反，该资源在 Market 的组织范围内变得可发现，成员可以在其中浏览和订阅它。

这种基于订阅的模型使每个成员能够控制他们的工作区。一个大型组织可能共享数十个连接器，但个别成员只订阅与他们的工作相关的连接器。
...
当为某个资源类型禁用审核时，已发布的资源会立即被成员发现——无需管理员操作。当启用审核时，资源进入 `pending_review` 状态，需要管理员批准后才能变为可见。

<Tip>
组织所有者自动绕过审核。他们发布的资源始终立即可被发现。
</Tip>

这种灵活性让组织能够匹配其治理需求。小型初创公司可能会禁用所有审核切换以实现无摩擦共享，而注重合规的企业可能会在智能体和连接器上启用审核以维持监督。

## 凭证回退

连接器和 MCP 服务器通常需要凭证（API 密钥、数据库密码、OAuth 令牌）。FIM One 提供了一个**回退机制**，使成员无需自己配置每个凭证。
...
- **启用回退** (`allow_fallback=true`，默认值)：未提供自己凭证的成员会自动使用所有者的默认凭证。这适用于团队共享的 API 密钥或内部服务，其中单个密钥覆盖整个团队。
...
FIM One 中的每个资源都有一个 `visibility`，用于确定其访问范围：
...
资源在您的工作区中可用，如果：
  1. 您拥有它（任何可见性），或
  2. 它已发布到您所属的组织，已获批准，且您已订阅它
```

<Warning>
将资源发布到组织不会自动授予访问权限。成员必须通过 Market 的组织范围订阅才能将资源添加到其工作区。
</Warning>
...
2. Alice 将其发布到她的团队组织（连接器禁用审核）
3. 该连接器在市场的组织范围内可被发现
4. Bob 浏览组织的共享资源，找到该连接器，并订阅
5. 该连接器出现在 Bob 的工作区中，使用 Alice 的数据库凭证作为后备
6. Carol 也订阅了。外部承包商 Dave 订阅并配置了自己的只读凭证
...
5. 如果发布者稍后编辑已批准的智能体，它会自动恢复到 `pending_review` 状态以供重新批准
...
- [市场架构](/concepts/market) — 了解全局市场及其与组织的关系。两者使用相同的订阅模型，但市场充当跨组织发现渠道，需要强制审查。
