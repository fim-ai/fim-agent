---
title: "上下文管理"
description: "FIM One 如何管理对话上下文 — 一个五层纵深防御体系，在防止 token 溢出的同时保持对话质量。"
---
...
FIM One 通过五层纵深防御架构来解决这个问题。每一层处理不同尺度的问题，它们可以干净地组合——没有哪一层需要做到完美，因为下一层会兜住它遗漏的部分。
...
WindowMemory 和 SummaryMemory 保留在代码库中，因为它们作为测试用的基础原语很有价值，也适用于不使用 web 层而直接嵌入 FIM One 核心库的用户。它们不是死代码——它们是架构成长起来的简单起点。
...
FIM One 使用启发式 token 估算而非真正的 tokenizer。这是一个有明确权衡的刻意选择。
