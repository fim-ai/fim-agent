...
**工作流**在流程已知且可重复时表现出色——"每周一，从 ERP 拉取发票、运行合规检查、将异常路由给审核人"。可视化编辑器让你将节点（智能体、连接器、知识库、LLM、HTTP、代码、人工审批、子工作流）连接到固定 DAG 中。

两种范式自然组合：工作流可以在需要灵活推理的任何步骤调用智能体，作为固定管道中的一部分。智能体不能直接调用工作流——关系是单向的。

<Tip>
**何时选择哪种：**如果你发现自己在为智能体编写非常具体的分步说明，那个流程可能应该在工作流中。如果任务需要判断、探索或适应意外数据，保持它作为智能体。
</Tip>
