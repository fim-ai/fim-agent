...
**에이전트**는 작업이 개방형일 때 탁월합니다 — "이번 분기 데이터를 분석하고 조치를 권장하세요." LLM이 즉시 계획하고 적응합니다.

**워크플로우**는 프로세스가 알려져 있고 반복 가능할 때 탁월합니다 — "매주 월요일마다 ERP에서 송장을 가져오고, 규정 준수 검사를 실행하고, 예외를 검토자에게 라우팅합니다." 비주얼 에디터를 사용하면 노드(에이전트, 커넥터, KB, LLM, HTTP, 코드, 사람 승인, 서브 워크플로우)를 고정 DAG로 연결할 수 있습니다.

두 패러다임은 자연스럽게 구성됩니다: 워크플로우는 고정 파이프라인 내에서 유연한 추론이 필요한 모든 단계에서 에이전트를 호출할 수 있습니다. 에이전트는 워크플로우를 직접 호출할 수 없습니다 — 관계는 단방향입니다.

<Tip>
**어느 것을 선택할지:** 에이전트에 매우 구체적이고 단계별 지침을 작성하고 있다면, 그 프로세스는 아마도 워크플로우에 속할 것입니다. 작업에 판단, 탐색 또는 예상치 못한 데이터에 대한 적응이 필요하면 에이전트로 유지하세요.
</Tip>
