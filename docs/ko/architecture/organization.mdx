...
조직(Organization)은 FIM One의 팀 협업 단위입니다. 사용자 그룹이 에이전트, 커넥터, 지식 베이스, MCP 서버, 워크플로우 및 스킬과 같은 리소스를 신뢰할 수 있는 범위 내에서 공유할 수 있게 해줍니다.

FIM One의 모든 리소스는 **개인용**(생성자에게만 표시됨)으로 시작됩니다. 리소스를 조직에 게시하면 **발견 가능**해져서 마켓의 조직 범위를 통해 다른 조직 구성원이 찾을 수 있습니다. 구성원들은 조직의 공유 리소스를 탐색하고 필요한 것을 구독합니다.
...
조직과 글로벌 마켓은 동일한 구독 기반 접근 모델을 공유합니다. 주요 차이점은 신뢰입니다. 조직은 구성원들이 서로를 알고 신뢰하는 팀이나 회사를 나타내므로 검토는 선택사항이며 자격증명 공유가 간단합니다.

## 조직 생성 및 관리

모든 사용자는 **무제한**의 조직을 생성하고 여러 조직에 참여할 수 있습니다. 조직에는 세 가지 역할이 있습니다:

| 역할 | 권한 |
|---|---|
| **Owner** | 전체 제어 — 멤버 관리, 설정 구성, 검토 우회 |
| **Admin** | 멤버 관리 및 게시된 리소스 검토 |
| **Member** | 공유된 리소스 탐색 및 구독 |

Owner는 항상 조직을 생성한 사용자입니다. 소유권은 이전할 수 있지만 공유할 수 없습니다.

## 리소스 게시

조직에 리소스를 게시하면 모든 구성원의 작업 공간에 **자동으로** 나타나지 않습니다. 대신 리소스는 Market의 조직 범위에서 검색 가능해지며, 구성원들이 이를 찾아보고 구독할 수 있습니다.

이 구독 기반 모델은 각 구성원에게 자신의 작업 공간을 제어할 수 있는 권한을 제공합니다. 대규모 조직은 수십 개의 커넥터를 공유할 수 있지만, 개별 구성원은 자신의 업무와 관련된 것들만 구독합니다.
...
검토는 **선택사항**이며 리소스 유형별로 구성됩니다. 각 조직은 독립적인 토글 플래그를 가집니다:
...
<Tip>
조직 소유자는 자동으로 검토를 우회합니다. 그들의 게시된 리소스는 항상 즉시 발견 가능합니다.
</Tip>
...
커넥터와 MCP 서버는 종종 자격증명(API 키, 데이터베이스 암호, OAuth 토큰)이 필요합니다. FIM One은 **폴백 메커니즘**을 제공하므로 멤버가 모든 자격증명을 직접 구성할 필요가 없습니다.
...
<Info>
자격증명 폴백은 멤버가 리소스를 구독한 후에만 적용됩니다. 폴백 메커니즘은 리소스의 접근성이 아닌 런타임에 자격증명이 어떻게 해결되는지를 결정합니다.
</Info>
...
```
다음의 경우 리소스를 워크스페이스에서 사용할 수 있습니다:
  1. 리소스를 소유하고 있거나 (모든 가시성), 또는
  2. 속한 조직에 게시되었고, 승인되었으며, 구독한 경우
```

<Warning>
조직에 리소스를 게시해도 자동으로 접근 권한이 부여되지 않습니다. 구성원은 Market의 조직 범위를 통해 구독하여 리소스를 워크스페이스에 추가해야 합니다.
</Warning>
...
4. Bob이 조직의 공유 리소스를 찾아보고 커넥터를 찾은 후 구독합니다
...
4. 에이전트가 검색 가능해집니다 — 다른 멤버가 이제 에이전트를 찾고 구독할 수 있습니다
...
4. 각 팀 멤버의 워크스페이스는 집중되고 정돈된 상태로 유지됩니다
