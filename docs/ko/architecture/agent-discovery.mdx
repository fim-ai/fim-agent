...
| MCP 서버 | `resolve_visibility()` — 모든 사용자 표시 | `MCPServerMetaTool` (점진적) |
| 에이전트 | `resolve_visibility()` — 모두 활성, 빌더 제외 | `call_agent` (카탈로그) |
| 내장 도구 | `discover_builtin_tools()` — 전체 집합 | 카테고리 필터 미적용 |
...
| 내장 도구 | `agent.tool_categories` 필터 | 카테고리별 부분집합 |

핵심 비대칭성: 커넥터와 지식 베이스는 에이전트로 범위가 지정되지만, 스킬, MCP 서버, CallAgent는 항상 전역입니다. 이는 설계 의도를 반영합니다 — 스킬은 조직 규칙입니다 (모두가 동일한 SOP를 따름), 반면 커넥터와 KB는 기능 바인딩입니다 (다양한 에이전트가 다양한 시스템에 연결됨).

## 모든 것이 도구입니다

LLM 수준에서 모든 리소스 유형은 호출 가능한 도구의 평면 목록으로 수렴됩니다. LLM은 커넥터, MCP 서버 또는 Knowledge Base를 호출하는지 여부에 대한 구조적 인식이 없습니다. `ToolRegistry`를 볼 수 있습니다 — 이름, 설명 및 매개변수 스키마가 있는 함수 집합입니다.

| 리소스 유형 | LLM 수준에서 변환됨 | 도구 이름 패턴 |
|---|---|---|
| Connector (점진적) | 단일 메타 도구 | `connector` |
| Connector (레거시) | 작업당 N개 도구 | `{connector}__{action}` |
| Database Connector (점진적) | 단일 메타 도구 | `database` |
| Database Connector (레거시) | 데이터베이스당 3개 도구 | `{db}__list_tables`, `{db}__describe_table`, `{db}__query` |
| MCP Server (점진적) | 단일 메타 도구 | `mcp` |
| MCP Server (레거시) | 서버당 N개 도구 | `{server}__{tool}` |
...
| 에이전트 자체 | 도구로 표시되지 않음 | _(지침 + 도구 조립)_ |

핵심 통찰: **에이전트는 도구가 아닙니다 — 도구를 사용하는 엔티티입니다.** 에이전트는 시스템 프롬프트에 지침을 제공하고 사용 가능한 도구를 결정합니다. 하지만 LLM의 관점에서는 "에이전트" 개념이 없습니다 — 시스템 프롬프트와 호출 가능한 함수 집합만 있습니다.
...
스킬은 두 가지 주입 모드를 지원합니다 -- **점진적** (기본값)과 **인라인** -- `SKILL_TOOL_MODE` 또는 에이전트의 `model_config_json.skill_tool_mode`로 제어됩니다. 점진적 모드에서는 컴팩트한 스텁만 시스템 프롬프트에 나타나고, LLM은 필요에 따라 `read_skill(name)`을 호출하여 전체 콘텐츠를 로드합니다. 이는 모든 리소스 유형에서 컨텍스트 소비를 최소화하는 FIM One의 광범위한 [점진적 공개](/architecture/progressive-disclosure) 아키텍처의 일부입니다.
