...
<AccordionGroup>
  ...
  <Accordion title="How do I report security vulnerabilities?">
    For **non-sensitive issues**, open a GitHub issue with the `[SECURITY]` tag in the title. Include reproduction steps, affected versions, and the potential impact.

    For **sensitive vulnerabilities** (e.g., credential exposure, authentication bypass), please email the team directly rather than filing a public issue. Contact information is available in the project repository.

    All security reports are triaged within 48 hours on business days.
  </Accordion>
</AccordionGroup>
