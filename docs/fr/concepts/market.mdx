...
<Info>
Le Marché utilise un **modèle de tirage** : les ressources sont découvertes par navigation et explicitement abonnées. Il n'y a pas de mécanisme d'adhésion automatique ou de poussée -- vous choisissez ce que vous installez, et vous pouvez filtrer par portée à tout moment.
</Info>
...
| **Serveur MCP** | Composant | Service d'outils tiers chargé dans les sessions |
...
Lorsque vous trouvez une ressource que vous souhaitez utiliser, cliquez sur **S'abonner**. Un assistant d'intégration vous guide à travers toute configuration requise -- par exemple, l'entrée des identifiants API pour un connecteur. Vous pouvez ignorer l'assistant et configurer les identifiants plus tard si vous préférez.

Une fois abonné :

- Les **agents** apparaissent dans votre sélecteur d'agents et dans le catalogue `call_agent`.
- Les **compétences** sont injectées automatiquement dans vos invites système.
- Les **workflows** apparaissent dans votre liste de workflows, prêts à être exécutés.
- Les **connecteurs** apparaissent dans votre ensemble d'outils et dans les menus déroulants de liaison d'agents.
...
|---|---|---|
| **Organisation** | Tous les membres de votre organisation | Non (confiance au niveau de l'organisation) |
| **Marché Global** | Tous les utilisateurs authentifiés | Oui -- approbation de l'administrateur requise |

La publication sur le Marché Global passe toujours par une porte de révision. Les administrateurs peuvent approuver, rejeter (avec une note) ou laisser la ressource en attente. Les ressources rejetées peuvent être révisées et soumises à nouveau.
...
Pour les Solutions qui regroupent des dépendances (par exemple, un Agent avec des Connecteurs liés et des Bases de connaissances), le processus d'abonnement résout et provisionne ces dépendances pour que tout fonctionne immédiatement.
