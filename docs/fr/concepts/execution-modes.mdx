...
| **Workflow** | DAG fixe défini au moment de la conception (éditeur visuel, 26 types de nœuds) | Chaînes d'approbation, ETL planifiés, automations multi-étapes |
...
Les **Workflows** excellent quand le processus est connu et répétable — « chaque lundi, récupérer les factures de l'ERP, exécuter les vérifications de conformité, acheminer les exceptions à un examinateur ». L'éditeur visuel vous permet de connecter des nœuds (Agent, Connecteur, KB, LLM, HTTP, Code, Approbation Humaine, Sous-Workflow) dans un DAG fixe.
...
<Tip>
**Quand choisir lequel :** Si vous vous trouvez à écrire des instructions très spécifiques, étape par étape, pour un Agent, ce processus appartient probablement à un Workflow. Si la tâche nécessite du jugement, de l'exploration ou l'adaptation à des données inattendues, gardez-la comme un Agent.
</Tip>
