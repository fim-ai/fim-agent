...
Chaque utilisateur peut créer des organisations **illimitées** et rejoindre un nombre quelconque d'entre elles. Une organisation a trois rôles :

| Rôle | Permissions |
|---|---|
| **Propriétaire** | Contrôle total — gérer les membres, configurer les paramètres, contourner l'examen |
| **Admin** | Gérer les membres et examiner les ressources publiées |
...
Lorsque vous publiez une ressource dans votre organisation, elle n'apparaît **pas** automatiquement dans l'espace de travail de chaque membre. Au lieu de cela, la ressource devient découvrable dans la portée organisation du Marché, où les membres peuvent la parcourir et s'y abonner.
...
Cette flexibilité permet aux organisations de correspondre à leurs besoins de gouvernance. Une petite startup pourrait désactiver tous les drapeaux d'examen pour un partage sans friction, tandis qu'une entreprise axée sur la conformité active l'examen sur les agents et les connecteurs pour maintenir une surveillance.

## Secours des Identifiants

Les connecteurs et les serveurs MCP nécessitent souvent des identifiants (clés API, mots de passe de base de données, jetons OAuth). FIM One fournit un **mécanisme de secours** afin que les membres n'aient pas à configurer chaque identifiant eux-mêmes.
...
- **Secours activé** (`allow_fallback=true`, par défaut) : Les membres qui ne fournissent pas leurs propres identifiants utilisent automatiquement les identifiants par défaut du propriétaire. Cela fonctionne bien pour les clés API partagées en équipe ou les services internes où une seule clé couvre toute l'équipe.
- **Secours désactivé** (`allow_fallback=false`) : Chaque membre doit configurer ses propres identifiants. Cela est approprié lorsque chaque utilisateur a besoin d'une clé API personnelle (par exemple, les licences SaaS par siège).
...
<Info>
Le secours des identifiants s'applique uniquement après qu'un membre s'abonne à la ressource. Le mécanisme de secours détermine la façon dont les identifiants sont résolus à l'exécution, et non si la ressource est accessible.
</Info>
...
<Warning>
Publier une ressource dans une organisation ne garantit pas un accès automatique. Les membres doivent s'abonner via la portée org du Marché pour ajouter la ressource à leur espace de travail.
</Warning>
...
2. L'équipe données s'abonne uniquement aux connecteurs de bases de données et aux API d'analyse
3. L'équipe marketing s'abonne uniquement aux connecteurs CRM et plateforme de messagerie
4. L'espace de travail de chaque membre de l'équipe reste concentré et bien organisé

## Voir aussi

- [Architecture du Marché](/concepts/market) — Pour le Marché mondial et sa relation avec les organisations. Les deux utilisent le même modèle d'abonnement, mais le Marché sert de canal de découverte inter-organisations avec examen obligatoire.
- [Découverte d'agent et de ressource](/architecture/agent-discovery) — Comment les ressources abonnées sont assemblées en ensembles d'outils lors du chat.
