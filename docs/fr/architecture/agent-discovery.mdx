...
| Connecteurs (BD) | `resolve_visibility()` — tous visibles pour l'utilisateur | `DatabaseMetaTool` (progressive) |
...
| Serveurs MCP | `resolve_visibility()` — tous visibles pour l'utilisateur | `MCPServerMetaTool` (progressive) |
| Agents | `resolve_visibility()` — tous actifs, non-constructeur | `call_agent` (catalogue) |
...
| Ressource | Découverte | Forme d'outil |
|---|---|---|
| Connecteurs | Seulement `agent.connector_ids` | `ConnectorMetaTool` ou par action hérité |
...
| Serveurs MCP | Portée utilisateur — **non contraint par l'Agent** | `MCPServerMetaTool` (progressive) |
...
L'asymétrie clé : les Connecteurs et les Bases de connaissances sont limités par l'Agent, mais les Compétences, les Serveurs MCP et CallAgent sont toujours globaux. Cela reflète l'intention de conception — les Compétences sont des règles organisationnelles (tout le monde suit les mêmes procédures), tandis que les Connecteurs et les BCs sont des liaisons de capacités (différents agents se connectent à différents systèmes).

## Tout est un outil

Au niveau du LLM, tous les types de ressources convergent en une liste plate d'outils appelables. Le LLM n'a pas de conscience structurelle de savoir s'il appelle un connecteur, un serveur MCP ou une base de connaissances. Il voit un `ToolRegistry` — un ensemble de fonctions avec des noms, des descriptions et des schémas de paramètres.

| Type de ressource | Devient au niveau du LLM | Modèle de nom d'outil |
|---|---|---|
...
L'idée clé : **un agent n'est pas un outil — c'est l'entité qui utilise les outils.** L'agent contribue ses instructions au prompt système et détermine quels outils sont disponibles. Mais du point de vue du LLM, il n'y a pas de concept « d'agent » — seulement un prompt système et un ensemble de fonctions appelables.
...
Les compétences supportent deux modes d'injection -- **progressif** (par défaut) et **inline** -- contrôlés par `SKILL_TOOL_MODE` ou par `model_config_json.skill_tool_mode` de l'Agent. En mode progressif, seuls des stubs compacts apparaissent dans le message système ; le LLM appelle `read_skill(name)` à la demande pour charger le contenu complet. Cela fait partie de l'architecture plus large [Progressive Disclosure](/architecture/progressive-disclosure) de FIM One qui minimise la consommation de contexte sur tous les types de ressources.
