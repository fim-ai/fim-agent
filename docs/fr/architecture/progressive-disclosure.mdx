...
icon: "filter"
description: "Comment FIM One minimise la consommation de contexte en chargeant les définitions d'outils à la demande -- stub en premier, détails sur demande."
---

## Le problème

Les LLM paient le contexte dans deux devises : les tokens et l'attention. Chaque définition d'outil injectée dans l'invite système coûte les deux. Un seul serveur MCP peut exposer 90+ outils. Cinq connecteurs API avec 20 actions chacun produisent 100 définitions d'outils. Trois connecteurs de base de données avec 30 tables chacun génèrent 90 autres descriptions de schéma. Avant même que l'utilisateur ne tape un mot, l'invite système peut consommer 50--100 Ko de contexte -- la moitié du budget d'un modèle 128K.
...
La solution naïve -- injecter tout, laisser le modèle le trier -- ne s'adapte pas. FIM One adopte l'approche opposée : **montrer au LLM le minimum dont il a besoin pour prendre une décision, et le laisser demander plus quand il en a besoin.**

## Le modèle

La divulgation progressive suit une architecture à deux niveaux sur tous les types de ressources :
...
L'insight clé : **les schémas d'outils complets sont limités à la conversation, non à l'invite.** Ils apparaissent en tant que messages de résultat d'outil que le système de gestion du contexte peut résumer ou tronquer dans les tours ultérieurs. En contraste, le contenu de l'invite système persiste dans toute la conversation à sa taille complète.

## Cinq mécanismes de divulgation

FIM One applique la divulgation progressive uniformément sur cinq types de ressources. Chacun utilise le même modèle à deux niveaux mais avec un méta-outil adapté à sa sémantique.
...
**Ce qui se passe à la demande :** Le LLM appelle `read_skill("Customer Complaint SOP")` et reçoit le texte SOP complet -- potentiellement des milliers de tokens d'instructions étape par étape, d'arbres de décision et de scripts intégrés. Ce contenu entre en tant que résultat d'outil, et non en tant que texte de prompt système, il est donc soumis à la gestion normale du contexte (résumé, troncature) dans les tours suivants.

**Mode hérité :** `SKILL_TOOL_MODE=inline` intègre le contenu complet de la Compétence directement dans le prompt système. Approprié lorsque vous avez peu de Compétences petites -- mais s'adapte mal.

**Économies de contexte :** Un déploiement avec 10 Compétences d'une moyenne de 2 000 tokens chacune consomme ~300 tokens en mode progressif (stubs uniquement) par rapport à ~20 000 tokens en mode inline. C'est une réduction de 98 % du coût de contexte persistant.

### Connecteurs API -- `connector`

**Ce que l'LLM voit initialement :**
...
Chaque stub de connecteur liste les noms d'actions mais pas les schémas de paramètres. L'LLM sait *quelles* actions existent mais pas *comment* les appeler -- c'est exactement le bon niveau de détail pour décider s'il faut utiliser un connecteur.

**Ce qui se passe à la demande :** `connector("discover", "salesforce")` retourne les schémas d'actions complets incluant les méthodes HTTP, les chemins URL, les schémas JSON des paramètres, et les modèles de corps de requête. `connector("execute", "salesforce", "get_contacts", {"limit": 10})` proxifie l'exécution via `ConnectorToolAdapter` avec injection complète d'authentification et journalisation d'audit.
...
**Économies de contexte :** Un connecteur avec 15 actions génère ~50 tokens de stub vs. ~3 000 tokens de schémas complets. Cinq connecteurs : ~250 tokens progressifs vs. ~15 000 tokens en mode hérité.

### Connecteurs de base de données -- `database`

**Ce que le modèle de langage voit initialement :**
...
Les stubs de base de données incluent les noms de tables (jusqu'à 10) et les comptages, donnant au modèle de langage suffisamment d'informations pour décider quelle base de données interroger sans charger aucun schéma de colonne.
...
Le flux en trois étapes reflète la façon dont un développeur explore une nouvelle base de données : parcourir les tables, inspecter le schéma, puis interroger. Le modèle de langage suit naturellement le même modèle.
...
Les serveurs MCP sont le cas le plus spectaculaire de divulgation progressive. Un serveur MCP GitHub expose 35+ outils. Un serveur de système de fichiers en expose 20+. Sans divulgation progressive, la connexion de 3 serveurs MCP pourrait injecter 70+ définitions d'outils dans l'invite système -- chacun avec des paramètres de schéma JSON complets.
...
**Fonctionnement :** Quand le nombre total d'outils dépasse `REACT_TOOL_SELECTION_THRESHOLD` (par défaut : 12), le moteur ReAct exécute un appel LLM léger pour sélectionner les 6 outils les plus pertinents pour la requête actuelle. Les outils restants sont stockés dans un registre complet. Un méta-outil `request_tools` est automatiquement enregistré, listant tous les outils non chargés comme un catalogue compact (nom + description de 80 caractères).
...
## Comment il s'intègre dans le pipeline d'assemblage d'outils
...
| **4. Chargement des connecteurs** | `ConnectorMetaTool` consolide tous les connecteurs API ; `DatabaseMetaTool` consolide tous les connecteurs BD |
...
| **8. Sélection à l'exécution** | Méta-outil `request_tools` enregistré lorsque la sélection filtre l'ensemble |
...
Une conception alternative aurait permis au framework d'étendre automatiquement les schémas d'outils en fonction d'heuristiques -- par exemple, en détectant quel connecteur concerne la requête de l'utilisateur et en injectant ses schémas avant que le LLM ne voie l'invite. FIM One a délibérément choisi l'approche pilotée par LLM pour trois raisons :

1. **Le LLM est meilleur pour la détection d'intention que les heuristiques.** Une requête comme « vérifier si le client a un ticket ouvert et mettre à jour son profil » implique deux connecteurs. La correspondance heuristique sur les mots-clés est fragile ; le LLM identifie naturellement les deux.
...
### Pourquoi des méta-outils par ressource plutôt qu'un outil universel ?

Un outil `discover_resource(type, name)` unique serait plus simple à implémenter mais moins efficace pour le LLM. Les méta-outils par ressource offrent :

- **Paramètres typés.** `connector` a `subcommand`, `connector`, `action`, `parameters`. `database` a `subcommand`, `database`, `table`, `sql`. Les schémas de paramètres indiquent au LLM exactement ce qui est attendu.
...
- **Sémantique de catégorie.** L'outil `connector` a la catégorie `connector`, `database` a la catégorie `database`, `mcp` a la catégorie `mcp`. Cela s'intègre au filtrage de catégorie d'agent -- un agent configuré avec uniquement la catégorie `connector` ne verra pas les méta-outils `database` ou `mcp`.

### Pourquoi à la fois les modes progressif et hérité ?

Tous les LLMs ne gèrent pas les méta-outils de la même manière. Les modèles plus petits ou affinés peuvent avoir du mal avec le modèle de découverte puis exécution en deux étapes. Le mode hérité fournit un repli direct où chaque action est un outil autonome avec son schéma complet visible -- aucune indirection de méta-outil requise.

La conception en mode dual soutient également la migration. Les déploiements existants peuvent basculer vers le mode progressif de manière progressive, en testant un type de ressource à la fois en changeant une seule variable d'environnement.
