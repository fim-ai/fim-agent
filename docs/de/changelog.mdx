...
- Exportzeitstempel respektieren nun die konfigurierte Zeitzone des Benutzers, anstatt rohe UTC anzuzeigen.
...
- Die Sitzungsablauf-Umleitung bewahrt nun die Abfragezeichenfolge, sodass Benutzer nach der erneuten Authentifizierung auf die genaue Registerkarte / den Filter zurückkehren, den sie angesehen haben, anstatt auf den bloßen Pfad.
...
- Feishu-Kanal-Setup-Hinweise duplizieren chinesische Kennzeichen nicht mehr, wenn die Benutzeroberfläche selbst bereits auf Chinesisch ist (z. B. zuvor gerendert "事件与回调 (事件与回调)").
- Feishu-Genehmigungskarten werden nach der ersten Entscheidung schreibgeschützt: Der `/callback`-Webhook gibt eine Ersatzkarte zurück, bei der die Schaltflächen Genehmigen/Ablehnen entfernt und die Kopfzeile grün (genehmigt) oder rot (abgelehnt) gefärbt ist, was wiederholte Klicks verhindert. Doppelte Klicks, die von veralteten Feishu-Clients ankommen, erhalten einen Toast "Diese Anfrage wurde bereits genehmigt/abgelehnt." und eine aktuelle Kopie der entschiedenen Karte, damit die veraltete Ansicht aufgeholt wird.
