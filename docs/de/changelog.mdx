...
- Exportzeitstempel respektieren nun die konfigurierte Zeitzone des Benutzers, anstatt die rohe UTC anzuzeigen.
...
- Die Umleitung bei Sitzungsablauf behält nun die Abfragezeichenfolge bei, sodass Benutzer nach der erneuten Authentifizierung auf die genaue Registerkarte / den Filter zurückkehren, den sie angesehen haben, anstatt auf den bloßen Pfad.
- Das Feishu-Kanalformular zeigt nicht mehr eine fehlerhafte Eingabeaufforderung "Nicht gespeicherte Änderungen verwerfen" an, wenn mit der Chat-Auswahl interagiert wird, die über dem Dialog überlagert ist.
...
- Die einfache Aktion "Testnachricht senden" auf Kanalzeilen und dem Detailblatt wurde wiederhergestellt. Der Approval Playground übt die vollständige Hook-Rundfahrt aus, aber ein reiner Benachrichtigungskanal (kein Genehmigungshook verdrahtet) benötigt immer noch eine schnelle Anmeldedaten-/Konnektivitätsprüfung, die der einfache Test-Send abdeckt.
