...
**"Thinking may not be enabled when tool_choice forces tool use"**
Anthropic thinking + forced function call. This is handled automatically by `structured_llm_call`'s fallback chain. If you see this in logs, it is informational — the call recovered at a lower level. No action needed.
