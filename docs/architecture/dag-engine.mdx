...
flowchart LR
    subgraph "Pre-execution"
        H["1. History summary"] --> EQ["enriched_query"]
    end
    subgraph "Planning"
        EQ --> P["2. DAGPlanner<br/>structured_llm_call"]
    end
    subgraph "Execution (per step)"
        TS["3. Tool Selection<br/>structured_llm_call"] --> RL["4. ReAct Loop<br/>chat()"]
        CG["5. ContextGuard<br/>compact"]
    end
    subgraph "Post-execution"
        PA["6. PlanAnalyzer<br/>structured_llm_call"] --> SS["7. stream_synthesize<br/>stream_chat()"]
    end
    P --> TS
    PA -->|"replan"| P
```
