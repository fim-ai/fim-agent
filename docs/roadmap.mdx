...
- [x] **Mid-stream quota cutoff is now a structured terminator, not a network error** — when a user's monthly token quota ran out partway through a streamed answer, the connection used to drop and the frontend reported a generic "网络错误", leaving no path to recovery. The streaming endpoints now emit a structured `error` SSE event with `code="QUOTA_EXCEEDED"`, current usage, monthly cap, next reset timestamp, and plan slug, and the playground surfaces a Dialog with an "Upgrade now" deep-link instead of a misleading toast. Critical correctness guarantee for the upcoming Stripe billing integration.
