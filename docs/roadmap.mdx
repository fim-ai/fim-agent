...
- [ ] **Agent Hook System**: A deterministic enforcement layer that runs **outside the LLM loop** — hooks execute automatically on tool events and cannot be bypassed by agent instructions. Three hook points: `PreToolUse` (validate / block before execution), `PostToolUse` (side effects after execution), `SessionStart` (inject dynamic context). Built-in hooks: auto-write `ConnectorCallLog` on every connector call (currently manual); block write operations when org is in read-only mode; auto-truncate oversized DB query results before they hit the agent; rate-limit per-connector call frequency. User-defined hooks: per-agent YAML config (`hooks:` field) declaring shell commands or Python callables triggered on matching tool events — a hook-based enforcement pattern shared across modern agent frameworks. Key design principle: **hooks are for "must always happen" logic that should never depend on the LLM remembering to do it**. Instructions say "record all calls"; hooks actually record them. Instructions say "don't write in read-only mode"; hooks actually block it.
  ...
  - [ ] **Per-hook config pass-through** — schema-level `{"name": ..., "config": {...}}` so per-agent `FeishuGateHook` overrides (timeout, poll interval, callback URL) work; bare-string `class_hooks` entries still supported
  - [ ] **DAG `tools_used` accuracy** — completion-notification cards currently derive `tools_used` from planner hints; surface actual tool names from per-step ReAct via DAG executor step-completion callback
  - [ ] **Hook inheritance for delegated sub-agents + Workflow `AGENT` nodes** — today `CallAgentTool` children and Workflow `AGENT` nodes spin up fresh ReActAgents with empty hook registry, silently bypassing `feishu_gate` on delegated sensitive calls. Decide inherit-by-default (secure) vs explicit opt-in (flexible); Eval Center always opts out (automation can't block on humans)
