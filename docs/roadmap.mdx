...
- [ ] **MCP Connection Pooling**: per-request STDIO subprocess spawning doesn't scale — 100 concurrent users = 100 subprocesses per MCP server. Pool STDIO connections with per-user env isolation (keyed by `(server_id, env_hash)`); SSE/HTTP transports share `httpx.AsyncClient` sessions. Target: under-100ms warm-start for pooled STDIO, O(1) HTTP connections per MCP server regardless of user count
#### Ecosystem & Scaling
